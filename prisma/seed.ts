import { PrismaClient, RoleType, Shift } from '@prisma/client'

const prisma = new PrismaClient()

const STORES = ['Магазин 1', 'Магазин 2', 'Магазин 3', 'Магазин 4', 'Магазин 5']

const DEFAULT_TASKS: { role: RoleType; shift: Shift; title: string }[] = [
  // Касиер - Първа смяна
...
const DEFAULT_PINS: { role: RoleType; pin: string }[] = [
...
async function main() {
  ...
  for (const storeName of STORES) {
    const store = await prisma.store.upsert({
      where: { id: STORES.indexOf(storeName) + 1 },
      update: {},
      create: { name: storeName },
    })

    for (const { role, pin } of DEFAULT_PINS) {
      await prisma.rolePin.upsert({
        where: { storeId_role: { storeId: store.id, role } },
        update: {},
        create: { storeId: store.id, role, pin },
      })
    }
    ...
    for (let i = 0; i < DEFAULT_TASKS.length; i++) {
      ...
      const existing = await prisma.task.findFirst({
        where: { storeId: store.id, role, shift, title },
      })
      if (!existing) {
        await prisma.task.create({
          data: { storeId: store.id, role, shift, title, order: i },
        })
      }
    }
